...
class Projects extends MX_Controller {
    ...
    function add()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == FALSE) // Validation ok
            {
                ...
                $_POST['items'] = serialize($this->input->post('items'));
    ...
    public function check_projectname(){
        $projectname = $this->input->post('check_projectname');
        $result = $this->db->get_where('projects',array('project_title'=>$projectname))->row_array();
        // print_r($result); exit;
        if(!empty($result)){
            echo 1; exit;
            // if($result['project_title'] == $projectname){
            //     echo 1; exit;
            // }else{
            //     echo 0; exit;
            // }
